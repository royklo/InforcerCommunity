...
- [ ] **JSON-01**: Structured JSON output with full depth (depth 100 per module convention)
- [ ] **JSON-02**: Organized by product → category → policy → sections (Basics, Settings, Assignments)

### CSV Output

- [ ] **CSV-01**: Flattened rows with context columns: Product, Category, PolicyName, SettingName, Value
- [ ] **CSV-02**: One row per setting value for Excel compatibility
