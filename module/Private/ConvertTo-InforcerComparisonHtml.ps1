...
.adv-logic-toggle { display:inline-flex; align-items:center; gap:2px; background:var(--bg); border:1px solid var(--border); border-radius:6px; padding:1px; font-size:10px; margin-left:4px; }
.adv-logic-toggle button { padding:2px 8px; border:none; background:none; font-size:10px; font-weight:600; color:var(--muted); cursor:pointer; border-radius:4px; transition:all 0.15s; font-family:inherit; }
.adv-logic-toggle button.active { background:var(--accent); color:#fff; }
...
'@
    ...
    [void]$sb.AppendLine('            }')
    [void]$sb.AppendLine('        }')
    [void]$sb.AppendLine('        var advChips = document.querySelectorAll("#active-adv-filters .adv-chip");')
    [void]$sb.AppendLine('        var matchesAdv = true;')
    [void]$sb.AppendLine('        if (advChips.length > 0) {')
    [void]$sb.AppendLine('            var logicToggle = document.getElementById("adv-logic-toggle");')
    [void]$sb.AppendLine('            var isOr = logicToggle && logicToggle.getAttribute("data-mode") === "or";')
    [void]$sb.AppendLine('            if (isOr) {')
    [void]$sb.AppendLine('                matchesAdv = false;')
    [void]$sb.AppendLine('                advChips.forEach(function(chip) {')
    ...
    [void]$sb.AppendLine('                    });')
    [void]$sb.AppendLine('                    if (!found) matchesAdv = false;')
    [void]$sb.AppendLine('                });')
    [void]$sb.AppendLine('            }')
    [void]$sb.AppendLine('        }')
    ...
    [void]$sb.AppendLine('    var lt = document.getElementById("adv-logic-toggle"); if (lt) lt.style.display = "none";')
    [void]$sb.AppendLine('    applyFilters();')
    [void]$sb.AppendLine('}')
    ...
    [void]$sb.AppendLine('    document.getElementById("adv-logic-toggle").style.display = "";')
    [void]$sb.AppendLine('    var menu = document.getElementById("adv-filter-menu");')
    ...
    [void]$sb.AppendLine('    chip.querySelector("input").focus();')
    [void]$sb.AppendLine('    applyFilters();')
    [void]$sb.AppendLine('}')
    ...
    [void]$sb.AppendLine('    });')
    [void]$sb.AppendLine('    if (!document.querySelectorAll("#active-adv-filters .adv-chip").length) { var lt = document.getElementById("adv-logic-toggle"); if (lt) lt.style.display = "none"; }')
    [void]$sb.AppendLine('    applyFilters();')
    [void]$sb.AppendLine('}')
    [void]$sb.AppendLine('function setAdvLogic(btn, mode) {')
    [void]$sb.AppendLine('    var wrap = document.getElementById("adv-logic-toggle");')
    [void]$sb.AppendLine('    wrap.querySelectorAll("button").forEach(function(b) { b.classList.remove("active"); });')
    [void]$sb.AppendLine('    btn.classList.add("active");')
    [void]$sb.AppendLine('    wrap.setAttribute("data-mode", mode);')
    [void]$sb.AppendLine('    applyFilters();')
    [void]$sb.AppendLine('}')
    if ($hasDuplicates) {
