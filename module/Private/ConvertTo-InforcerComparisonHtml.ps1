...
.score-bar-fill { height: 100%; border-radius: 999px; width: 0%; transition: width 0.05s linear, background-color 0.3s ease; }
...
'@
    ...
    [void]$sb.AppendLine('        if (pct < 40) return "#ef4444";')
    [void]$sb.AppendLine('        if (pct < 60) return "#d97706";')
    [void]$sb.AppendLine('        if (pct < 80) return "#eab308";')
    ...
    [void]$sb.AppendLine('    var colors = ["#059669","#34d399","#fbbf24","#3b82f6","#8b5cf6","#ec4899","#f97316","#06b6d4"];')
    ...
    [void]$sb.AppendLine('    // Fire from left and right sides')
    [void]$sb.AppendLine('    for (var i = 0; i < 150; i++) {')
    [void]$sb.AppendLine('        var side = i < 75 ? 0 : 1;')
    [void]$sb.AppendLine('        pieces.push({')
    [void]$sb.AppendLine('            x: side === 0 ? 0 : canvas.width,')
    [void]$sb.AppendLine('            y: canvas.height * 0.6,')
    [void]$sb.AppendLine('            vx: (side === 0 ? 1 : -1) * (Math.random() * 15 + 5),')
    [void]$sb.AppendLine('            vy: -(Math.random() * 20 + 8),')
    [void]$sb.AppendLine('            color: colors[Math.floor(Math.random() * colors.length)],')
    [void]$sb.AppendLine('            size: Math.random() * 8 + 4,')
    [void]$sb.AppendLine('            rotation: Math.random() * 360,')
    [void]$sb.AppendLine('            rotSpeed: (Math.random() - 0.5) * 15,')
    [void]$sb.AppendLine('            gravity: 0.4,')
    [void]$sb.AppendLine('            opacity: 1,')
    [void]$sb.AppendLine('            shape: Math.random() > 0.5 ? "rect" : "circle"')
    [void]$sb.AppendLine('        });')
    [void]$sb.AppendLine('    }')
    ...
    [void]$sb.AppendLine('            p.vx *= 0.99;')
    [void]$sb.AppendLine('            if (p.y > canvas.height * 0.7) p.opacity -= 0.02;')
    ...
    [void]$sb.AppendLine('            if (p.shape === "rect") { ctx.fillRect(-p.size/2, -p.size/4, p.size, p.size/2); }')
    [void]$sb.AppendLine('            else { ctx.beginPath(); ctx.arc(0, 0, p.size/2, 0, Math.PI*2); ctx.fill(); }')
