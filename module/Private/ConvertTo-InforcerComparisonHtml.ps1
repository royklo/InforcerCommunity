...
'@
    ...
    [void]$sb.AppendLine('}')
    [void]$sb.AppendLine('// PowerShell syntax highlighting for script code blocks')
    [void]$sb.AppendLine('function highlightPS(code) {')
    [void]$sb.AppendLine('    var h = code.innerHTML;')
    [void]$sb.AppendLine('    // Comments (# to end of line)')
    [void]$sb.AppendLine('    h = h.replace(/(#(?!region|endregion)[^\n]*)/g, ''<span class="ps-comment">$1</span>'');')
    [void]$sb.AppendLine('    h = h.replace(/(&lt;#[\s\S]*?#&gt;)/g, ''<span class="ps-comment">$1</span>'');')
    [void]$sb.AppendLine('    // Strings')
    [void]$sb.AppendLine('    h = h.replace(/(&quot;[^&]*?&quot;|&#39;[^&]*?&#39;)/g, ''<span class="ps-string">$1</span>'');')
    [void]$sb.AppendLine('    // Variables')
    [void]$sb.AppendLine('    h = h.replace(/(\$[\w:]+)/g, ''<span class="ps-variable">$1</span>'');')
    [void]$sb.AppendLine('    // Keywords')
    [void]$sb.AppendLine('    var kw = "if|else|elseif|foreach|for|while|do|switch|try|catch|finally|throw|return|function|param|begin|process|end|filter|class|enum|using|trap|break|continue|exit";')
    [void]$sb.AppendLine('    h = h.replace(new RegExp("\\b(" + kw + ")\\b", "gi"), ''<span class="ps-keyword">$1</span>'');')
    [void]$sb.AppendLine('    // Cmdlet-style names (Verb-Noun)')
    [void]$sb.AppendLine('    h = h.replace(/\b([A-Z][a-z]+(?:-[A-Z][a-zA-Z]+)+)\b/g, ''<span class="ps-cmdlet">$1</span>'');')
    [void]$sb.AppendLine('    // Type accelerators [Type]')
    [void]$sb.AppendLine('    h = h.replace(/(\[[A-Za-z][\w.]*\])/g, ''<span class="ps-type">$1</span>'');')
    [void]$sb.AppendLine('    code.innerHTML = h;')
    [void]$sb.AppendLine('}')
