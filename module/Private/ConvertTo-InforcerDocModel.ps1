...
function Get-InforcerPolicyDisplayInfo {
    ...
    .DESCRIPTION
        ...
        admins recognise from the Microsoft admin portals. Returns a hashtable with FriendlyName and
        Category, both $null when no mapping matches.
    #>
    [CmdletBinding()]
    param(
    ...
    # Entra ID settings (policyTypeId 12) — internal name -> admin portal path
    $entraSettingsMap = @{
    ...
    $entraTypeMap = @{
        62 = @{ FriendlyName = $null; Category = 'Authentication methods' }   # Certificate-based auth
    ...
    $friendlyName = $null; $category = $null
    if ($Product -eq 'Entra' -and $PolicyTypeId -eq 12 -and $entraSettingsMap.ContainsKey($PolicyName)) {
        $m = $entraSettingsMap[$PolicyName]; $friendlyName = $m.FriendlyName; $category = $m.Category
    }
    elseif ($Product -eq 'Entra' -and $entraTypeMap.ContainsKey($PolicyTypeId)) {
        $m = $entraTypeMap[$PolicyTypeId]; if ($m.FriendlyName) { $friendlyName = $m.FriendlyName }; $category = $m.Category
    }
    elseif ($Product -eq 'SharePoint' -and $PolicyTypeId -eq 14 -and $sharepointSettingsMap.ContainsKey($PolicyName)) {
        $m = $sharepointSettingsMap[$PolicyName]; $friendlyName = $m.FriendlyName; $category = $m.Category
    }
    elseif ($Product -eq 'M365 Admin Center' -and $PolicyTypeId -eq 16 -and $m365SettingsMap.ContainsKey($PolicyName)) {
        $m = $m365SettingsMap[$PolicyName]; $friendlyName = $m.FriendlyName; $category = $m.Category
    }
    @{ FriendlyName = $friendlyName; Category = $category }
}
