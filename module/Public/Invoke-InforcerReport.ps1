...
param(
    ...
    [ArgumentCompleter({
        ...
        $known = @(
            'ActiveUserCount','Assessment','CopilotAdoption','GetDetectedRisks','GetRiskyUsers',
            'IntuneDeviceCompliance','IntuneAppInventory','MailFlowSummary','ShadowAiDetection',
            'TenantAuditReport','UserSignInActivity'
        )
        $candidates = if ($script:InforcerReportTypeCache) {
            ...
            foreach ($entry in $script:InforcerReportTypeCache) {
                $v = $entry.PSObject.Properties['key'].Value -as [string]
                if ($v) { [void]$tmp.Add($v) }
            }
            $tmp
        } else { $known }
        $matches = [System.Collections.Generic.SortedSet[string]]::new([System.StringComparer]::OrdinalIgnoreCase)
        foreach ($c in $candidates) { if ($c -like "$wordToComplete*") { [void]$matches.Add($c) } }
    ...
    [ArgumentCompleter({
        ...
        if ($script:InforcerReportTypeCache -and $boundReport.Count -gt 0) {
            foreach ($rt in $boundReport) {
                $rtStr = $rt -as [string]
                $entry = $null
        ...
        $matches = [System.Collections.Generic.SortedSet[string]]::new([System.StringComparer]::OrdinalIgnoreCase)
        foreach ($f in $formats) { if ($f -like "$wordToComplete*") { [void]$matches.Add($f) } }
...
foreach ($run in $runs) {
    ...
    Write-Progress -Activity 'Invoke-InforcerReport' -Status "Polling run $runIdValue ($completedRuns/$totalRuns)" `
        -PercentComplete (($completedRuns - 1) / [Math]::Max($totalRuns, 1) * 100)
    ...
    foreach ($out in $outputs) {
        ...
        try {
            $download = Invoke-InforcerRawDownload -Endpoint $downloadEndpoint -DefaultFileName $defaultName -ErrorAction Stop
        } catch {
...
Write-Progress -Activity 'Invoke-InforcerReport' -Completed
